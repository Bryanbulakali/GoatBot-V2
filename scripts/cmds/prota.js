...
module.exports = {
  config: {
    ...
    version: "2.1",
    description: "Quiz prota avec réponses multiples",
    category: "🎮 Jeu"
  },
  ...
  onChat: async function ({ event, message, usersData }) {
    ...
    if (body === "!stop") {
      ...
      return message.reply("🛑 Quiz Protagoniste arrêté.");
    }

    // START
    if (session.status === "waiting" && body === "!start") {
      const filePath = path.join(__dirname, "..", "data", "prota.json");

      if (!fs.existsSync(filePath)) {
        delete activeSessions[threadID];
        return message.reply("❌ Fichier prota.json introuvable.");
      }
      ...
      try {
        ...
        delete activeSessions[threadID];
        return message.reply("❌ Erreur de lecture dans prota.json");
      }

      if (!Array.isArray(questions) || questions.length === 0) {
        delete activeSessions[threadID];
        return message.reply("❌ Aucune question trouvée dans prota.json.");
      }
