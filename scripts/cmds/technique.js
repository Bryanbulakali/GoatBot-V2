...
module.exports = {
  config: {
    ...
    version: "2.1",
    description: "Quiz des techniques des mangas et leur utilisateur avec réponses multiples",
    category: "🎮 Jeu"
  },
  ...
  onChat: async function ({ event, message, usersData }) {
    ...
    if (session.status === "waiting" && body === "!start") {
      const filePath = path.join(__dirname, "..", "data", "tech.json");

      if (!fs.existsSync(filePath)) {
        delete activeSessions[threadID];
        return message.reply("❌ Fichier tech.json introuvable.");
      }
      ...
      try {
        ...
        delete activeSessions[threadID];
        return message.reply("❌ Erreur de lecture dans tech.json");
      }

      if (!Array.isArray(questions) || questions.length === 0) {
        delete activeSessions[threadID];
        return message.reply("❌ Aucune question trouvée dans tech.json.");
      }
